...
Client.prototype.call = function () {
  ...
  this._call(methodName, parameters, function (err, result) {
    var time = Date.now() - startTime;

    if (err) {
      self.emit('error', 'error', methodName, time, err);
    }else {
...
Client.prototype.login = function (params, callback) {
  var self = this;
  this._call('login', params, function (error, user) {
    if(error) {
...
Client.prototype._getLoginParams = function () {
  ...
  if(auth && auth.password && auth.email) {
    return [{"password":auth.password,"user":{"email":auth.email}}];
  }
};
